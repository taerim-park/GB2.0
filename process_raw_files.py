...
def report(aename, now):
    ...
    missing=0
    for i in range(0, 50): 
        ofname = datetime.strftime(now - timedelta(seconds=i), "%Y-%m-%d-%H%M%S")
        if os.path.exists(f'{raw_path}/{ofname}'): 
            #print(f'init ofname ok {ofname}')
            break
    if ofname == "": print('PANIC:  can not...')
